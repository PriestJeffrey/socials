# Pulseboard - Phase 4 exit (draft)

**Status:** Implementation complete on `v1-core` - awaiting Human UAT. See [docs/v1-core-ship.md](v1-core-ship.md).  
**Date:** 2026-07-30  
**Note:** `/x` added to middleware PROTECTED (gstack remediation).

## Delivered

- `xAdapter` stub (no OAuth / publish; `manualCopy`)
- `/x` compose + clipboard + honest banner
- Settings X section (no Connect)
- Nav → X; Health phase 4 + X row
- Tests: capability flags + reject OAuth/publish

## Human UAT

1. Open **X** in nav → see “Not auto-publish”
2. Type draft → Copy → paste elsewhere works
3. Settings has no X Connect button
4. Reply when ready to sign Phase 4 / continue
