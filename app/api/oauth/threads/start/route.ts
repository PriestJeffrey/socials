...
export async function GET() {
  ...
  if (!cfg.configured) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=" +
          encodeURIComponent(
            "Set THREADS_APP_ID/SECRET (or META_*) or THREADS_USE_FIXTURES=true",
          ),
        appUrl,
      ),
    );
  ...
  try {
    const th = getAdapter("threads");
    if (!th) throw new Error("Threads adapter missing");
    const url = await th.beginOAuth(user.id);
