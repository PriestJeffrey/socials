...
export async function GET() {
  ...
  const limit = await rateLimiter.check(`oauth:li:start:${user.id}`, 10, 15 * 60 * 1000);
  if (!limit.ok) {
  ...
  if (!cfg.configured) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=" +
          encodeURIComponent(
            "Set LINKEDIN_CLIENT_ID/SECRET or LINKEDIN_USE_FIXTURES=true",
          ),
        appUrl,
      ),
    );
  ...
  try {
    ...
    return NextResponse.redirect(await li.beginOAuth(user.id));
  } catch (err) {
