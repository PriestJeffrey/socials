...
export async function GET() {
  ...
  if (!cfg.configured) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=" +
          encodeURIComponent(
            "Set VIMEO_CLIENT_ID/SECRET or VIMEO_USE_FIXTURES=true",
          ),
        appUrl,
      ),
    );
  ...
  try {
    const vimeo = getAdapter("vimeo");
    if (!vimeo) throw new Error("Vimeo adapter missing");
    const url = await vimeo.beginOAuth(user.id);
