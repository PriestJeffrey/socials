...
export async function GET() {
  ...
  if (!cfg.configured) {
    return NextResponse.redirect(
      new URL(
        "/settings?error=" +
          encodeURIComponent(
            "Set PINTEREST_APP_ID/SECRET or PINTEREST_USE_FIXTURES=true",
          ),
        appUrl,
      ),
    );
  ...
  try {
    const pin = getAdapter("pinterest");
    if (!pin) throw new Error("Pinterest adapter missing");
    const url = await pin.beginOAuth(user.id);
